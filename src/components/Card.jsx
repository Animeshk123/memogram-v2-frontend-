...
export default function Card({src,name,text,userImage,likes,comments,created,id}){
  ...
  const [classN,setClass] = useState("h-48");

const load = () => {
  setTimeout(function() {setClass("h-auto")});
  ...
  return(
    ...
    <Screen state={screen} text="Please Wait..."/>
     ...
     <div className="flex items-center jusitify-between py-2">
      <button onClick={() => {countLikes()}} disabled={isLiked} type="button" className={`${(isLiked) ? "bg-blue-500 text-white" : "bg-gray-200 text-black"} outline-none w-1/2 mx-1 rounded-full py-3 px-4 flex items-center justify-center cursor-pointer`}>
      {length}
