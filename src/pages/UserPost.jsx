...
export default function UserPost({setProgress}) {
    ...
    try{
      ...
      let api = await fetch(`${BaseUrl}/api/v1/comment`,{
        method:"POST",
          ...
          body:JSON.stringify({author:(currentUser.displayName) ? currentUser.displayName : currentUser.email,text:commentRef.current.value,postId:data._id})
      });
      let res = await api.json();
      setProgress(70);
      getData();
  ...
  const deleteComments = async (commentId) => {
    try{
      ...
      let api = await fetch(`${BaseUrl}/api/v1/delete/comment`,{
        ...
        body:JSON.stringify({postId:id,commentId})
      });
  ...
  return (
    ...
    {
      data ? (<Show id={data._id} src={data.post} name={data.user.name} text={data.description} userImage={data.user.profileUrl} likes={data.likes} comments={data.comments} created={data.createdAt}/>) : (<Loader/>)
    }
    <div className="mt-6">
    <p className="text-lg my-3">Top Comments</p>
      {
      (data) ? 
        ((!data.comments.length > 0) ? (<p className="">No comments</p>) :
        (
          data.comments.map((com,i) => {
            ...
            <p className="capitalize">{com.text}</p>
            <span onClick={() => {deleteComments(com.commentId)}} className="material-symbols-outlined absolute bottom-0 right-0 p-2 cursor-pointer font-bold">delete</span>
            </div>
            </div>)
          })
        )) : ""
