...
export default function Dashboard({setProgress}){
  ...
  const getData = async (user) => {
    try{
      ...
      setTimeout(function() {  setData({post:data.post,totalLikes:Likes,totalComments:Comments});}, 400);
  ...
  return (
    <>
      <div className="container mx-auto px-4 mt-4">
        ...
        <div className="w-full mt-4 myClass">
        {
          data ? ((data.post.length > 0) ? 
          (data.post.map((d,i) => {
            return (<SCard key={i} deleteFunc={deletePost} likes={d.likes} src={d.post} id={d._id}/>)
          })) : (<h1 className="w-full flex items-center justify-center block mt-3">No Posts</h1>)) : (
          <>
           <Loader/>
           <Loader/>
           <Loader/>
